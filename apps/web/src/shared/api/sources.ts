import type { DatabaseConnection, TestDatabaseConnectionResult, UpsertDatabaseConnection } from "@/types/api";
...
export async function testSource(token: string, data: UpsertDatabaseConnection): Promise<TestDatabaseConnectionResult> {
  return apiRequest<TestDatabaseConnectionResult>("/sources/test", {
    method: "POST",
    body: JSON.stringify(data),
    token,
  });
