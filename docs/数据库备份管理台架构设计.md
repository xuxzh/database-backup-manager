...
- `PG_ISREADY_PATH`：PostgreSQL 连接测试工具路径，默认 `pg_isready`。

对于达梦、Oracle、SQL Server 等客户端授权或安装复杂的数据库，架构预留外挂客户端目录：
...
此模式下访问 Vite 默认地址 `http://127.0.0.1:5173`。Vite 会将 `/api` 代理到 `http://127.0.0.1:8080`，用于前端热更新开发。生产环境和 Docker Compose 不使用 Vite 开发服务器。
