...
flowchart LR
    ...
    API --> Config["配置管理模块"]
    API --> Runs["运行记录模块"]
    API --> ManualUpload["手动上传模块"]
    API --> Scheduler["任务调度器"]
    Scheduler --> Executor["备份执行器"]
    ManualUpload --> TargetAdapter
    ManualUpload --> LocalFS
...
- 调度器负责按计划触发任务。
- 备份执行器负责一次完整备份流程。
- 手动上传模块负责无法直连数据源场景下的已有备份包接收、校验、manifest 生成和远端上传，不执行数据库导出。
...
此模式下访问 `http://127.0.0.1:8080`。Rust 服务会托管 `apps/web/dist/`，适合验证接近生产环境的单体部署效果。
...
Docker 构建阶段会自动安装前端依赖并执行 `pnpm build`，最终运行镜像只复制 Rust 二进制和 `apps/web/dist/` 静态资源。
...
Vite 开发服务器的 API 代理属于开发期配置，通过 `apps/web/.env` 中的 `VITE_API_PROXY_TARGET` 设置；未配置时默认代理到 `http://127.0.0.1:8080`。
...
- 校验文件大小和扩展名。
- 将上传文件写入本地临时目录。
- 计算 checksum 并生成 manifest。
- 调用目标适配器上传文件并验证远端文件存在。
- 使用运行记录和运行日志展示上传进度。

手动上传不依赖数据源连接，也不调用数据库客户端工具。它适用于数据库服务器无法被管理台直接访问，或需要由外部流程先完成数据库导出的场景。

数据源通过 `backup_mode` 区分自动备份和手动备份。`automatic` 数据源需要维护主机、端口、用户名和密码，并支持连接测试和数据库列表读取；`manual` 数据源仍需维护主机和端口作为来源标识，但不要求维护数据库凭据，也不需要连接测试。手动上传页面的来源下拉来自数据源列表，后端根据选中的数据源写入来源名称和数据库类型。
...
`remote_path` 记录成功上传后的远端归档文件路径。运行记录页面可以基于该路径下载或删除远端备份文件；删除文件只标记 `file_deleted` 和 `file_deleted_at`，不删除运行记录和执行日志。

自动备份运行记录会保存执行当时的任务、数据源、数据库和备份目标快照，用于在任务被改名、改目标或删除后继续解释历史记录。快照不包含密码、密钥、用户名等敏感信息；旧版本运行记录允许这些快照字段为空。
...
- 页面可以按任务和运行记录查看日志。
- 通过运行记录删除远端备份文件时，应写入 `file_delete` 阶段日志。
...
```text
<base-dir>/scheduled/<source-server>/<db-type>/<database-name>/<yyyy-mm-dd>/
<base-dir>/manual/<source-label>/<db-type>/<database-name>/<yyyy-mm-dd>/
```

`scheduled/` 表示管理台自动或手动触发任务后由执行器导出的备份；`manual/` 表示用户上传的已有备份包。旧版本已经生成的远端路径不自动迁移，运行记录中的 `remote_path` 仍按原值下载或删除。
...
- `apps/web` 放 React 前端工程，生产构建产物输出到 `apps/web/dist/`。
- `apps/e2e` 放 Playwright 端到端测试。
